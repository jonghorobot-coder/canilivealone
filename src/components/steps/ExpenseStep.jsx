...
const CATEGORY_ICONS = {
  housing: (
    <svg className="w-5 h-5 sm:w-6 sm:h-6 text-[#0F3D2E]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export function ExpenseStep() {
  ...
  return (
    ...
    >
      {/* 요약 카드 */}
      <div className="bg-white/[0.07] backdrop-blur-md rounded-2xl p-4 sm:p-5 border border-white/[0.12] animate-stagger animate-stagger-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-5 sm:gap-8">
            <div>
              <span className="text-[11px] sm:text-[12px] text-white/40 block mb-1.5 font-medium uppercase tracking-wider">수입</span>
              <span className="text-[20px] sm:text-[22px] font-bold text-white tabular-nums tracking-tight">
                {incomeNum.toLocaleString()}<span className="text-[13px] sm:text-[14px] text-white/40 ml-0.5 font-normal">만원</span>
              </span>
            </div>
            <div className="h-9 sm:h-10 border-l border-white/[0.12]" />
            <div>
              <span className="text-[11px] sm:text-[12px] text-white/40 block mb-1.5 font-medium uppercase tracking-wider">지출</span>
              <span className="text-[20px] sm:text-[22px] font-bold text-white tabular-nums tracking-tight">
                {totalExpenses.toLocaleString()}<span className="text-[13px] sm:text-[14px] text-white/40 ml-0.5 font-normal">만원</span>
              </span>
            </div>
          </div>
          <div className="text-right">
            <span className="text-[11px] sm:text-[12px] text-white/40 block mb-1.5 font-medium uppercase tracking-wider">잔액</span>
            <span className={`text-[22px] sm:text-[26px] font-bold tabular-nums tracking-tight ${
              balance >= 0 ? 'text-[#6EE7B7]' : 'text-[#FCA5A5]'
            }`}>
              {balance >= 0 ? '+' : ''}{balance.toLocaleString()}<span className="text-[13px] sm:text-[14px] ml-0.5 font-normal opacity-70">만원</span>
            </span>
          </div>
        </div>
      </div>
      ...
      {/* 수입 입력 카드 */}
      <div className="bg-white rounded-2xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-neutral-100/80 p-4 sm:p-5 animate-stagger animate-stagger-3">
        <div className="flex items-center gap-3 sm:gap-4">
          <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-[#E8F3EF] to-[#D4EBE3] flex items-center justify-center flex-shrink-0">
            <svg className="w-5 h-5 sm:w-6 sm:h-6 text-[#0F3D2E]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[15px] sm:text-[16px] font-semibold text-neutral-800 tracking-[-0.01em]">월 수입</p>
            <p className="text-[13px] sm:text-[14px] text-neutral-400 mt-0.5">세후 실수령액</p>
          </div>
        ...
        {isIncomeZero && (
          <p className="mt-3 text-responsive-base text-red-500 font-semibold">수입을 입력해주세요</p>
        )}
      </div>
      ...
      <div className="bg-white/5 rounded-2xl p-4 border border-white/10 animate-stagger animate-stagger-10">
        <div className="flex items-start gap-3">
          <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/10 flex items-center justify-center flex-shrink-0 mt-0.5">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-white/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          </div>
          <div>
            <p className="text-responsive-md text-white/80 font-semibold">
              아래 항목은 건너뛰어도 괜찮아요
            </p>
            <p className="text-responsive-base text-white/50 mt-1">
      ...
      {!optionalValid && (
        <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200">
          <div className="flex items-center gap-3">
            ...
            <p className="text-responsive-md text-amber-700 font-semibold">
      ...
      {balance < 0 && isIncomeFilled && (
        <div className="p-4 sm:p-5 rounded-2xl bg-red-50 border border-red-200">
          <div className="flex items-start gap-3 sm:gap-4">
            ...
            <div>
              <p className="text-responsive-xl text-red-700 font-bold">
                월 {Math.abs(balance).toLocaleString()}만원 적자
              </p>
              <p className="text-responsive-md text-red-500 mt-1">
